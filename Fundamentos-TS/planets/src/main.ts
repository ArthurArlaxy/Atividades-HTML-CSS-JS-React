...
class Planet{
    name:string;
    coordenada:[number,number,number,number];
    situacao:situacao
    satelites: string[];
    constructor(name:string,coordenada:[number,number,number,number], situacao:situacao,) {
        this.name = name 
...
function createPlanet(){
    const name:string = prompt('Digite o nome do planeta: ')
    const coordenadaStr:string = prompt(`Digite a coordenada [numbers-numbers-numbers-numbers]:`)
    ...
    const situacaoInput:string = prompt(`Digite a situação do planeta:
        habitado,
        habitavel,
        inabitavel,
        inexplorado`)
    ...
    const situacoesValidas: situacao[] = ["habitado", "habitavel", "inabitavel", "inexplorado"];
    if (!situacaoInput || !situacoesValidas.includes(situacaoInput as situacao)) {
        return alert('Situação inválida! Escolha entre: habitado, habitavel, inabitavel, inexplorado.');
    }

    let newplanet = new Planet(name, [coordenada[0],coordenada[1],coordenada[2],coordenada[3]],situacaoInput as situacao)
    planets.push(newplanet)
    alert(`O planeta ${newplanet.name} foi adicionado com sucesso`)
}

function getPlanet(name){
    const planet:Planet = planets.find(p => p.name === name)
    return planet
}
...
function alterSituation(){
    ...
    const situacaoValidas:situacao[] = ['habitado','habitavel','inabitavel','inexplorado']
    if(!situacaoInput || !situacaoValidas.includes(situacaoInput as situacao)){
        return alert('Situação inválida! Escolha entre: habitado, habitavel, inabitavel, inexplorado.');
    }

    planet.situacao = situacaoInput as situacao
...
function removeSatelite(){
    ...
    const sateliteIndex:number = planet.satelites.indexOf(sateliteInput)

    if(!sateliteInput||!planetName){
        return alert('Escreva o nome inválido');
    }

    if(sateliteIndex === -1)return alert('Satélite não encontrado')
...
function showPlanets(){
    let text:string = 'Listagem de planetas:\n\n'
