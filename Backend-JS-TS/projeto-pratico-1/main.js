...
async function userInteraction() {
    ...
    const answer = await askFilePath(`Informe o camino do arquivo de saída (padrão: ${defaultName}: )`)
...
function run() {
    if (process.argv.length >= 4){
        ...
        console.log("-------------------------")
        console.log("HTML Tag Escaper 1.0")
        console.log("-------------------------")
        console.log("Argumentos não citados no argumento! Favor informar os caminhos para realizar a conversão ")
        userInteraction()
    }
}
